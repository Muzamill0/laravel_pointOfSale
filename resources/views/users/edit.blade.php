...
<div class="container card ">
    <div class="col-md-12">
        ...
        @endif
        <form action="{{ route('edit.user', $user->id) }}" method="POST" class=" p-3">
            @csrf
            ...
            <div class="col-md-12">
                ...
                <input class="form-control" type="text" id="email" name="email"
                    value="{{ old('email') ? old('email') : $user->email }}" placeholder="Enter your Email">
                @error('email')
                    <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
            <div class="col-md-12">
                ...
                <select name="role" id="role"
                    ...
                    @foreach ($roles as $db_role)
                        <option value="{{ $db_role->id }}"
                           @if(old('role')) {{ old('role') == $db_role->id ? 'selected' : '' }}
                           @else
                           @foreach ($user->roles as $role)
                           {{ $db_role->id == $role->id ? 'selected' : '' }}
                           @endforeach
                            @endif >{{ $db_role->name }} </option>
                    @endforeach
                </select>
                @error('role')
                    <p class="text-danger">{{ $message }}</p>
